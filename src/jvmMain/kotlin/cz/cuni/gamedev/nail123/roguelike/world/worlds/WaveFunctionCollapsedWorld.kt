...
class WaveFunctionCollapsedWorld: DungeonWorld() {
    ...
    fun getSameNeighbouringTiles(helperMap: Map<Position3D, HelperMapTile>, position: Position3D): (List<Position3D>, List<Position3D>) {
        ...
        while (openVertices.isNotEmpty()) {
            ...
            Pathfinding.fourDirectional(currentPos.position)
            ...
            Pathfinding.fourDirectional(currentPos.position)
                .filter { helperMap[it].type != tileType }
                .forEach {
                    visitedVertices.
                }
        }
    ...
    fun getNeighboursOfSameNeighbouringTiles(helperMap: Map<Position3D, HelperMapTile>, position: Position3D): List<Position3D> {
